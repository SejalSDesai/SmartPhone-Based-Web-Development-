...
//            .whereField("buyerId", isNotEqualTo: currentUserID)
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            ...
            let product = products[indexPath.row]
            deleteProductFromFirestore(productID: product.title, indexPath: indexPath)
        }
    }

    // ...

    func deleteProductFromFirestore(productID: String, indexPath: IndexPath) {
        let productsCollection = db.collection("products")
        
        productsCollection.document(productID).delete { [weak self] error in
            if let error = error {
                print("Error deleting product: \(error.localizedDescription)")
                // Handle the error, if any
            } else {
                // Successfully deleted from Firestore, now update the local array
